...
public class InventoryService {
	...
	public String getResponse(long bakeryId) {
		...
		String prompt = "너는 빵집 재고 분석 AI이다. 반드시 한국어로 답변하라.\n"
			...
			+ "재고 정보(stocks): {breadId quantity YYMMDD, breadId quantity YYMMDD, ...}\n"
			+ "\n"
			+ "출력 규칙:\n"
			+ "\n"
			+ "추세 분석:\n"
			+ "1. breadId별 quantity 추세(증가/감소)를 분석하라.\n"
			+ "2. 다른 breadId보다 추세 변화가 두드러지면 아래와 같이 출력하라.\n"
			+ "✅ {breadId}가 {date}부터 [증가/감소]하는 추세입니다.\n"
			+ "\n"
			+ "재고 부족:\n"
			+ "1. breadId의 quantity 총합이 0이거나 현저히 낮으면 출력하라.\n"
			+ "✅ {breadId}는 재고가 거의 남지 않는 것으로 보입니다.\n"
			+ "\n"
			+ "재고 과잉:\n"
			+ "1. breadId의 quantity 총합이 다른 breadId보다 현저히 높으면 출력하라.\n"
			...
			+ "\n"
			+ "주의사항:\n"
			+ "1. 모든 최종 출력에서 {breadId}는 breadIdList의 name으로 변경하라.\n"
			+ "2. 데이터가 부족하다는 메시지를 출력하지 말 것\n"
			+ "3. 모든 수치는 구체적인 숫자로 표현할 것\n"
			+ "\n"
			+ "최종 출력 형식:\n"
			+ "현재상황\n"
			+ "{1, 2, 3번 분석 결과}\n"
